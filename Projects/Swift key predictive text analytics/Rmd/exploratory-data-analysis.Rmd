...
rm(top_25_nt, common_words, top_25_bt, top_25_bn, all_mc, all_m, all_texts, all_twitter, all_news, all_blogs, all_dm, all_tdm)
```
...
text_associations = findAssocs(text_tdm, "year", 0.10)
...
ggplot(text_associations_df, aes(score, word)) +
  geom_point(size = 3) + 
  theme_minimal()
rm(text_associations, text_associations_df)
```
...
wordcloud::wordcloud(bi_tokens, freq, max.words = 15, colors = cividis(n = 3))
```


```{r clean-up-bigrams, echo = F, error=F, warning=F}
rm(bigram_m, bigram_tdm, bi_tokens, freq)
```
...
wordcloud::wordcloud(tri_tokens, freq, max.words = 15, colors = cividis(n = 3))
```

```{r clean-up-wordcloud, echo = F, error=F, warning=F}
rm(trigram_m, trigram_tdm, tri_tokens, freq)
```

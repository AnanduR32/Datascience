...
we'd get the same result as we'd if the intercept estimator $\beta_{0}$  was under consideration 
...
same result still, furthmore, the $\beta$ coefficient can be calculated 
...
deviation of intercept from reference when $X_{2}=1$
...
Linear regression $\mathrm{response}_{i}=b_{0}+b_{1}\mathrm{predictor}_{i}+e_{i}$   
Logistic regression $\mathrm{log}\left(\frac{\mathrm{Pr}(response_{i}|predictor_{i},b_{0},b_{1})}{1-\mathrm{Pr}(response_{i}|predictor_{i},b_{0},b_{1})}\right)=b_{0}+b_{1}predictor_{i}$  
If, score==0 then we have  
$\mathrm{log}\left(\frac{\mathrm{Pr}(response_{i}|predictor_{i},b_{0},b_{1})}{1-\mathrm{Pr}(response_{i}|predictor_{i},b_{0},b_{1})}\right)=b_{0}$   
Thus,  
- $b_{0}$ - Log odds of Ravens win if they score zero points  
- $b_{1}$ - Log odds ratio of win probability for each point scored(relative to the zero point)  
- $exp(b_{1})$ - Odds ratio of win probability for each point scored(relative to the zero point)

and $\frac{e^{b_{0}}}{1+e^{b_{0}}}$ is the probability of whether raven's win with score "0"  

To find the unit increase in probability of winning,  
- Let $b_{0}+b_{1}(predictor_{i})$ be the probability of winning for given score, and     
- $b_{0}+b_{1}(predictor_{i}+1)$" style="display:inline be the probability of winning for given score + 1, which means the probability given unit increase in score.    

Subtracting the terms we get, $b_{0}+b_{1}(predictor_{i}+1)-b_{0}+b_{1}(predictor_{i})=b_{1}$
